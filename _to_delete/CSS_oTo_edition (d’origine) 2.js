...
navbottom : function (objid) {	
	...
	// var hauteurObj = obj.offsetHeight();
	...
	if (newmarge_H >= (0 - hauteurObj))
		...
		make_scrolling : function (liobj) {
			...
			if (obj_parent.nodeName.toLowerCase() == 'li')
			{
				...
				/*	if (lf == 0)
					{
						rapport += liparents.length+'<br>'+celifrere.parentNode.getElementsByTagName('a')[0].innerHTML+' ?== '+obj_parent.parentNode.getElementsByTagName('a')[0].innerHTML+'<br><br>';
					}
				*/	
					if (celifrere.parentNode.getElementsByTagName('li')[0].innerHTML == obj_parent.parentNode.getElementsByTagName('li')[0].innerHTML)
					{
						rapport += 'meme UL, '+celifrere.parentNode.getElementsByTagName('a')[0].innerHTML+' == '+obj_parent.parentNode.getElementsByTagName('a')[0].innerHTML+'<br>';
						if (celifrere.getElementsByTagName('a')[0].innerHTML != obj_parent.getElementsByTagName('a')[0].innerHTML)
						{
						//	celifrere.getElementsByTagName('ul')[0].style.display = 'none';
							
							var ulsdeceli = celifrere.getElementsByTagName('ul');
							for (uli=0; uli<ulsdeceli.length; uli++)
								ulsdeceli[uli].style.display = 'none';
							
							rapport += celifrere.getElementsByTagName('a')[0].innerHTML+' != '+liobj.getElementsByTagName('a')[0].innerHTML+' : ferme LI UL<br>';
						}
						else
						{
							celifrere.getElementsByTagName('ul')[0].style.display = 'block';
							rapport += celifrere.getElementsByTagName('a')[0].innerHTML+' != '+liobj.getElementsByTagName('a')[0].innerHTML+' : OUVRE LI UL<br>';
						}
					...
					{
				//		celifrere.getElementsByTagName('ul')[0].style.display = 'none';
				//		rapport += 'AUTRE UL, '+celifrere.parentNode.getElementsByTagName('a')[0].innerHTML+' != '+obj_parent.parentNode.getElementsByTagName('a')[0].innerHTML+'<br>';
				//		rapport += 'donc fin: '+lf+'/'+liparents.length+'<br>'+celifrere.parentNode.getElementsByTagName('a')[0].innerHTML+' == '+obj_parent.parentNode.getElementsByTagName('a')[0].innerHTML+'<br>';
		...
		//	document.getElementById('Suivi').innerHTML = obj_parent.nodeName.toLowerCase()+'<br>---'+rapport+'---<br><br><br>'+document.getElementById('Suivi').innerHTML;
			...
			if (typeof(id_infospg_ul) == 'undefined')
				id_infospg_ul = 0;
			...
			if (size_h_IP > 0)
			{
				if (objul.id == '')
				{
					var good_ulid = 'infospg_ul'+id_infospg_ul;
					objul.id = good_ulid; 
				}
				else
...
//		alert ('ul no : '+ul+'\nposition = '+pos_element.join(' x ')+'\nsize = '+size_element.join(' x ')+'\nposition basse = '+pos_v_bas+',  max possible en bas = '+bas_v_max);
				...
				if (pos_v_bas > bas_v_max)
				{
				 // Essaie par au-dessus : 
					...
					{
		// alert (pos_v_ob_sidessus+' > '+max_v_siaudessus+' : assez de place au dessus...');
					...
					else
					{
						...
						{
		// alert (hauteur_dispo_haut+' > '+hauteur_dispo_bas+' : pas assez de place au dessus, plus de place en bas que en haut');
															
					//		objul.style.height = (hauteur_dispo_bas-10)+'px';																					
						}
						else
						{
							var heightobj = hauteur_dispo_haut-20;
							objul.style.marginTop = '-'+(heightobj+10)+'px';
							
//	 alert (hauteur_dispo_haut+' > '+hauteur_dispo_bas+' : pas assez de place au dessus, plus de place en haut...'+heightobj);
						}
						...
						if (memo_nav_created['Menu_Nav_'+good_ulid] == null)
						{
							...
							divgobottom.onmousedown = function () { var obj=this.parentNode.id.replace(/^Menu_Nav_/, ''); document.bouge = setInterval("CSS_OTO_EDITOR.form.navbottom(\""+obj+"\");", 42 ) };
							divgotop.onmousedown = function () { var obj=this.parentNode.id.replace(/^Menu_Nav_/, ''); document.bouge = setInterval("CSS_OTO_EDITOR.form.navtop(\""+obj+"\");", 42 ) };
							...
							var marginleft_MNav = left_ul - left_li;
							
							if (objul.parentNode.nodeName.toLowerCase() == 'div' && objul.parentNode.getElementsByClassName('Menu_Nav')[0] == null)
							{
								divnavul.style = 'position:absolute; z-index:10; top:0px; left:0px; margin-left:'+(marginleft_MNav-25)+'px; margin-top:0px; border:2px solid #F00;';
								objul.parentNode.appendChild (divnavul);
								
							}
							else if (objul.parentNode.getElementsByClassName('Menu_Nav')[0] == null)
							{
								divnavul.style = 'margin-left:'+(marginleft_MNav-25)+'px;';
...
//				alert (leidobj+'\n'+idclass_liste_polices_champs+'\n-'+leobj_ok_value+'-');
	...
	//		alert (obj.id);
		 ...
		 explain_cette_value_liste : function (prop_name, typevalue)
		 {
			...
			//	 alert (lesimg+'\n'+prop_name+'\n'+typevalue);
				 ...
				 for (yy= 0; yy<lesimg_T.length; yy++)
				 {
					 ...
					 {
						 infovoulue += '<li class="explain">'+voulue+'</li>';
					 }
				 }
				 
				 if (infovoulue != '')
					infovoulue = '<ul>'+infovoulue+'</ul>';
			...
			if (self.menu_add_property == null || self.lang_ex != self.lang)
			{
				...
				for (categ in base_menu)
				{
					...
					for (prop_name = 0;  prop_name < base_menu[categ].length; prop_name++)
					{
						...
						{
					//	alert (prop_name);
							liste_props += '<li><a href="#MemeEndroit" onclick="cssproperty_add(this)">'+base_menu[categ][prop_name]+'</a><ul><li class="explain"><p>'+self.navigators_img(base_menu[categ][prop_name])+'</p><p>'+self.explain_1line(base_menu[categ][prop_name])+'</p></li></ul></li>';
						}
				...
				menu1 += '<li><a href="#MemeEndroit">'+self.translate('+ property')+'</a><ul>'+menu_props+'</ul></li>';
				menu1 += '<li><a href="#MemeEndroit" onclick="if(confirm(\''+self.translate('Do you really want to delete the ___property___ property ?')+'\')) CSS_OTO_EDITOR.form.cssproperty_suppr(this);">'+self.translate('- delete ___property___')+'</a></li>';
				...
				new_menu_props = new_menu_props.replace(/(id="InfosPage")/, '$1 onselectstart="return false;" onmousedown="CSS_OTO_EDITOR.form.sort_liste_valeur(this);"');
		...
		aff_menu_properties : function (obj)
		{
			var id_obj = obj.id;
			
			document.getElementById(id_obj).getElementsByTagName('span')[0].style.display = 'none'; 
			
			var new_menu = self.oTo_html2DOM('<div id="CSS_oTo_form">'+self.menu_add_property+'</div>');
	...
	// /(<div class="select_color">.+(?!<div class="select_color">))$/
									...
									if (is_combine) 
									{
										...
										if (liste_iddiv_choix_vals.length > 1)
										{
											...
											{	
												var classa_name = '';
												if (liste_iddiv_choix_vals[aae] == liselected)
							...
							HTML_form_return += '<div id="css'+def_css_idx+'_property_btn'+css_property_idx+'_btnplus" onmouseover="CSS_OTO_EDITOR.form.aff_menu_properties(this);"><span class="plus">&bull;</span></div>';
